...
> 方法：缩小 2D floorplan 使自然利用率达到 50%（cell area 429µm² → die 32×32µm），而非在超大面积上用 density flag 强制压缩。

![2D(50%) vs 3D](../figures/gcd_2d_vs_3d_typ.png)

| | Die 面积 | HPWL |
|---|---|---|
| 2D (50% util) | 32×32 µm | **2.85 M** |
| 3D (per die) | 63×58 µm | **5.53 M** |
| 3D / 2D | — | **193%** |

**各自在最优密度下，3D 的绝对 HPWL 仍高于 2D。** 2D 能把所有单元紧密排在一层，3D 的跨层连接物理距离是额外开销。
