...
301 个实例在 2D 和 3D 中名称完全相同，可精确做对位位移计算。
...
**单元整体向原点收缩。** 原因：2D die 面积 89×82 µm，3D 每 Die 只有 63×58 µm。2D 中处于远端边角的单元被"拉回"到较小的 3D 边界内。

### 距离-位移相关性

**r = 0.919**（极强正相关）。离 2D 中心越远的单元，位移越大。——几乎是面积压缩的必然结果，而非 placement 策略差异。
...
**完全相同。** placer 的层指派与 2D 空间上的邻近性无关——Die 分配几乎是随机的（除了总线信号）。
...
Die1 略高，可能因为单元密度更高（213 vs 142 的逻辑分配）。
