...
| p | 1（完全图假设） | 0.698 |
| 收益 | 17% | 21% — 量级一致 |
| 收益来源 | d₂≥5 长线被垂直折叠替代 | 超长尾（>50µm）被消除；中位数不变 |
| 跨 Die 网线占比 | ~50% | 71% |
| 短线行为 | 不受影响 | **受损（−27%）** |

geometry 的 p=1 假设是"每对单元间都有一根线"的极端互联场景，在这个场景下长线折叠确实成立。但 gcd 的 p=0.698 更接近真实电路——连线稀疏得多。placer 在这种条件下选择了一条不同的路径：先把单元物理压缩（r=0.919），顺便消除超长线，而非逐对做垂直折叠。

两种机制在 gcd 上恰好产出相似数字（21% vs 17%），但本质不同。geometry 给的是"如果互联密度极高，折叠的潜力上限是多少"，实测给的是"在真实互联密度下，placer 实际走了哪条路径"。
