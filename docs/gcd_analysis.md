...
geometry 的 17% 是 p=1 完全图假设下的理想值——在极致互联密度下，垂直折叠确实起效。但在 gcd 这种稀疏设计上，placer 没有走这条路。两个机制恰好产出相似数字，但本质不同。

**对大设计的潜在含义**：如果收益主要靠密度，那么芯片越大、单元越稀疏，3D 的 HPWL 优势可能被稀释。需要 jpeg（23K 实例）验证。
