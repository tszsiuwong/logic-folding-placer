...
| TAO § | TAO 的核心问题 | PD+EDA 侧的对应事务 | 当前状态 |
|-------|--------------|-------------------|---------|
| §1.2 | 细粒度折叠是否缩短关键路径？ | 2D/3D placement 逐网线 HPWL 对比 | ✅ gcd |
| §2.2 步骤 2 | 分割方案的质量如何？ | 层分布、信号组保留、HBT 密度 | ✅ gcd |
| §3.1 | 批处理观测粒度失配 | DEF/Verilog 提取逐网线/逐单元数据 | ✅ 静态 |
| §3.3 | 单一真源 | DEF+Verilog+partition 按实例名 join | ✅ gcd 100% |
| §2.2 步骤 5 | 键合点资源约束 | HBT 局部密度、溢流分析 | ✅ gcd |
| §3.2 | 增量传感器、事务 API | 需要 placer 增量接口 | ❌ |
| §2.2 步骤 4 | 3D 布局跨层目标函数 | 同引擎 2D/3D 公平对比 | ❌ 待解决 |
| §5 | 后端环 / 端到端环 | 离线 vs 在线闭环 | ❌ |
