...
TAO 文档 §2.4 描述了三方协作的架构——人、Agent、EDA 算法在同一张层指派表上以显式分级协作，"人负责外层动作——布设模块级 tier 意图、审查 hint 改写 diff、批准 locked 升级"。§5.2 的端到端环也描述了"前端变异 → 编译 → 综合 → 增量物理实现 → 真实 PPA 归因回源码"的回路。

这些描述中，**架构师的判断和决策是不可替代的一环。** 我们能提供的是更精确的物理反馈数据——但数据本身不产生决策。架构师看到数据后决定改不改标注、改哪里、锁不锁——这个判断不取决于工具改进。
